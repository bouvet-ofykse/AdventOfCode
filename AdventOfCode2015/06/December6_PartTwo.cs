...
{
    public static long CalculateBrightnessOfLights()
    {
        List<Action> actions = [];
        var lights = new Dictionary<(int y, int x), int>();
        ...
        return lights.Sum(light => light.Value);
    }

    private static void PerformAction(Action action, Dictionary<(int y, int x), int> lights)
    {
        for (var y = action.From.y; y <= action.To.y; y++)
        {
            for (var x = action.From.x; x <= action.To.x; x++)
            {
                ...
                {
                    Intstruction.TurnOn => lights[(y, x)] + 1,
                    Intstruction.TurnOff => Math.Max(0, lights[(y, x)] - 1),
                    Intstruction.Toggle => lights[(y, x)] + 2,
                    _ => lights[(y, x)]
                };
            }
        }
