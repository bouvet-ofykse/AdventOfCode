using AdventOfCode2024._1;
using AdventOfCode2024._10;
using AdventOfCode2024._11;
using AdventOfCode2024._13;
using AdventOfCode2024._14;
using AdventOfCode2024._19;
using AdventOfCode2024._2;
using AdventOfCode2024._3;
using AdventOfCode2024._4;
using AdventOfCode2024._5;
using AdventOfCode2024._6;
using AdventOfCode2024._7;
using AdventOfCode2024._8;
using AdventOfCode2024._9;

// Day 1
// Console.WriteLine($"December 1 - Part 1: {December1_PartOne.CalculateDistance()}");
// Console.WriteLine($"December 1 - Part 2: {December1_PartTwo.CalculateSimilarity()}");

// Day 2
// Console.WriteLine($"December 2 - Part 1: {December2_PartOne.CalculateSafeReports()}");
// Console.WriteLine($"December 2 - Part 2: {December2_PartTwo.CalculateSafeReports()}");

// Day 3
// Console.WriteLine($"December 3 - Part 1: {December3_PartOne.CalculateResults()}");
// Console.WriteLine($"December 3 - Part 2: {December3_PartTwo.CalculateResults()}");

// Day 4
// Console.WriteLine($"December 4 - Part 1: {December4_PartOne.FinNumberOfXmasAppearences()}");
// Console.WriteLine($"December 4 - Part 2: {December4_PartTwo.FinNumberOfXmasAppearences()}");

// Day 5
// Console.WriteLine($"December 5 - Part 1: {December5_PartOne.CalculateSafeUpdates()}");
// Console.WriteLine($"December 5 - Part 2: {December5_PartTwo.CalculateSafeUpdates()}");

// Day 6
// Console.WriteLine($"December 6 - Part 1: {December6_PartOne.PredictGuardsRoute()}");

// Day 7
// Console.WriteLine($"December 7 - Part 1: {December7_PartOne.CalculateResults()}");
// Console.WriteLine($"December 7 - Part 2: {December7_PartTwo.CalculateResults()}");

// Day 8
// Console.WriteLine($"December 8 - Part 1: {December8_PartOne.CalculateUniqeLocationsOfAntinode()}");
// Console.WriteLine($"December 8 - Part 2: {December8_PartTwo.CalculateUniqeLocationsOfAntinode()}");

// Day 9
// Console.WriteLine($"December 9 - Part 1: {December9_PartOne.CalculateFilesystemChecksum()}");
// Console.WriteLine($"December 9 - Part 1: {December9_PartTwo.CalculateFilesystemChecksum()}");

// Day 10
// Console.WriteLine($"December 10 - Part 1: {December10_PartOne.CalculateScoresOfAllTrailheads()}");
// Console.WriteLine($"December 10 - Part 2: {December10_PartTwo.CalculateScoresOfAllTrailheads()}");

// Day 11
// Console.WriteLine($"December 11 - Part 1: {December11_PartOne.SumNumberOfStonesAfterBlinks()}");
// Console.WriteLine($"December 11 - Part 2: {December11_PartTwo.SumNumberOfStonesAfterBlinks()}");

// Day 12

// Day 13

// Day 14
// Console.WriteLine($"December 14 - Part 1: {December14.CalculateBathroomSaftyFactor()}");

// Day 14
Console.WriteLine($"December 19 - Part 1: {December19_PartOne.CalculateNumberOfPossibleDesigns()}");